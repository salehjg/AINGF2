...
xst -ifn example_ila1.xst -ofn example_core.log -intstyle silent
REM Check Results
IF ERRORLEVEL 0 GOTO PASS_XST
ECHO An error occurred running XST on example_ila1
GOTO END_IMPLEMENT_FAIL
:PASS_XST
...
COPY ..\..\ila1.ngc        .\results\
COPY example_ila1.ngc        .\results\
COPY chipscope_icon.ncf .\results\
COPY ..\..\ila1.ncf        .\results\
cd .\results

REM Run ngdbuild
ECHO 'Running NGDBUILD'
ngdbuild -uc ..\..\example_design\example_ila1.ucf -p xc3s400-pq208-4 -sd . example_ila1.ngc example_ila1.ngd
REM Check Results
IF ERRORLEVEL 0 GOTO PASS_NGDBUILD
ECHO An error occurred running NGDBUILD on example_ila1
GOTO END_IMPLEMENT_FAIL
:PASS_NGDBUILD
...
REM Run map
ECHO 'Running MAP'
map -w -p xc3s400-pq208-4 -o example_ila1.map.ncd example_ila1.ngd
REM Check Results
IF ERRORLEVEL 0 GOTO PASS_MAP
ECHO An error occurred running MAP on example_ila1
GOTO END_IMPLEMENT_FAIL
:PASS_MAP

REM Run par
ECHO 'Running PAR'
par -w -ol high example_ila1.map.ncd example_ila1.ncd 
REM Check Results
IF ERRORLEVEL 0 GOTO PASS_PAR
ECHO An error occurred running PAR on example_ila1
GOTO END_IMPLEMENT_FAIL
:PASS_PAR
REM Report par results

ECHO 'Running design through BITGEN'
bitgen -d -g GWE_cycle:Done -g GTS_cycle:Done -g DriveDone:Yes -g StartupClk:Cclk -w example_ila1.ncd
REM Check Results
IF ERRORLEVEL 0 GOTO PASS_BITGEN
ECHO An error occurred running BITGEN on example_ila1
GOTO END_IMPLEMENT_FAIL
:PASS_BITGEN
